...
async function main(): Promise<void> {
  ...
  console.log(`Running baseline agents over ${toProcess.length} event(s)${limit ? ` (--limit=${limit})` : ""}...`);
  ...
  for (const [i, item] of toProcess.entries()) {
    ...
    const decision =
      item.agent === "cart_abandonment"
        ? await decideCartAbandonmentBaseline(customer, item.event)
        : item.agent === "subscription_recovery"
          ? await decideSubscriptionRecoveryBaseline(customer, item.event)
          : await decideDisputeResponderBaseline(customer, item.event);
  ...
  writeFileSync(join(RESULTS_DIR, "baseline_decisions.json"), JSON.stringify(decisions, null, 2) + "\n", "utf-8");
  ...
  console.log(`Wrote ${decisions.length} decisions to ${join(RESULTS_DIR, "baseline_decisions.json")}`);
