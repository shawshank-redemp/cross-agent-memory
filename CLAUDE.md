...
- ~24% "normal" customers — ONE recovery-eligible event, cart or subscription only, and no other history. The negative control. Two constraints keep it genuinely silent, both learned by getting it wrong: (a) **no dispute domain** — an unresolved dispute fires `disputeCautionWarranted` and tightens the memory arm's cap while the baseline stays at the default, so roughly a third of the control diverged between arms by construction; dispute-agent coverage comes from `repeat_offender_dispute` and `cross_domain_risk` instead. (b) **`paid_count` pinned to 1** — `successful_payment_count` reads the latest cycle's `paid_count`, so a cycle drawn at 2+ would fire `provenPayer` and widen the cap, the same divergence arriving from the subscription side. It also used to emit each domain's clean TERMINAL state (paid cart, active cycle, conceded dispute), all ineligible for a decision, which would have erased the cohort entirely once the runner started filtering. What makes it a control is the absence of HISTORY, not the absence of a recovery question. `validate:data` runs the real `computeMemorySignals` over every member and asserts all eight memory-derived signals sit at their defaults.
...
Measured before/after on the committed batch (700 customers, 1,720 eligible
events): **215 firings under the old rule, 131 under the new one**. The entire 84-firing gap is `cross_agent_gaming`,
